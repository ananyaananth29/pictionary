...
class Game {
 ...
 async initScene(data) {// we add data to accept the obj named this.game.scene.start
...
//async==>interact with database, its a js promise (async means asynchronous)
  ...
  async joinOrCreateGame(id) {//this is to create a new game or join a game
    //this is a wrapper function we are creating to avoid writing promise, as createGame function is asynchronous
    try{
    let auth = await game.authenticate();//if a id already exists, then dont create a new id ...continue as the old game
    ...
    if(result==null){
    result = await game.createGame(id, auth.id);//if there is no id running...create a new game with a new name and is
    }
    return result;
  }catch(e){
      console.error(e);
